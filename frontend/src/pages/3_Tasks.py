...
with tabs[0]:
    ...
    try:
        ...
        for project in projects:
            projects_dict[project.get("id")] = project
            try:
        ...
        if all_tasks:
            ...
            if table_action:
                ...
                if table_action["action"] == "edit_status":
                    ...
                    can_update_status = (
                        task_assignee_id == user_id or
                        str(task_assignee_id) == str(user_id)
                    )
                    ...
                    if not can_update_status:
                        ...
                        if st.button("Update Status", key=f"btn_update_task_{selected_task.get('id')}", use_container_width=True):
                            try:
                                updated_task = task_service.update_status(selected_task.get("id"), new_status)
                    ...
                    task_item = table_action.get("item")
                    st.markdown("---")
                    ...
                    try:
                        ...
                        is_task_project_owner = project_data and project_data.get("owner_id") == user_id
                        can_reassign_this_task = user_role in [UserRole.ADMIN, UserRole.MANAGER] or is_task_project_owner
                        
                        if not can_reassign_this_task:
                            ...
                            current_assignee_id = task_item.get("assignee_id")
                            ...
                            if current_assignee_id and current_assignee_id != "Unassigned":
                                current_user = next((u for u in all_users if u.get("id") == current_assignee_id), None)
                                if current_user:
                                    current_selected = f"{current_user.get('name', 'Unknown')} ({current_user.get('email', 'Unknown')})"
                            
                            if not current_selected:
                                current_selected = "Unassigned"
                            ...
                            if st.button("Reassign Task", key=f"btn_reassign_{task_item.get('id')}", use_container_width=True):
                                try:
                                    updated_task = task_service.reassign_task(task_item.get("id"), new_assignee_id)
                                    show_success(f"Task reassigned successfully!")
                                    if table_action_key in st.session_state:
                                        del st.session_state[table_action_key]
                                    st.session_state.tasks_cache_cleared = True
                                    st.rerun()
